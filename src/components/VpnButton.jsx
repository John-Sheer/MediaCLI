...
export default function VpnButton({ torActive, onToggle }) {
  ...
  return (
    <button
      ...
      className={`relative flex items-center gap-1.5 px-2 py-1.5 rounded-md font-mono text-[9px] font-medium tracking-wider transition-all duration-300 active:scale-[0.96] border ${
        status
    ...
    >
      ...
      {status ? <Shield className="w-3 h-3 text-green-400" /> : <ShieldOff className="w-3 h-3" />}
