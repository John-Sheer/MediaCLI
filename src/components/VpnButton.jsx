...
export default function VpnButton({ torActive, onToggle }) {
  ...
  return (
    <button
      ...
      className={`relative flex items-center gap-2 px-3.5 py-2 rounded-md font-mono text-[11px] font-medium tracking-wider transition-all duration-300 active:scale-[0.96] border ${
        status
          ? "bg-[#040406] border-green-500/30 text-white/90 shadow-[0_0_16px_-4px_rgba(34,197,94,0.2)]"
          : "bg-[#040406] border-[#ff3b5c]/25 text-white/30 hover:text-white/50 hover:border-[#ff3b5c]/40"
      }`}
    ...
    >
      ...
      {status ? <Shield className="w-3.5 h-3.5 text-green-400" /> : <ShieldOff className="w-3.5 h-3.5" />}
      <span>{status ? "VPN ACTIF" : "VPN INACTIF"}</span>
    </button>
  );
}
